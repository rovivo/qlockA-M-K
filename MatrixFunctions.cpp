...
// Voreinstellungen ändern
	#define LeftRight				// links rechts vertauscht
	#define UpsideDown				// Oben und unten vertauscht
	#define LeftRightCable			// RGB LEDs von links nach rechts verkabelt (kürzester Weg)
...
void generateClockMatrix(PATTERN* matrix, unsigned short h, unsigned short m) {
	...
	//// Einzelne Minuten (1-4) // nur wenn minuten auch RGB LEDs
	//switch(m%5){
...
void writeNeo(PATTERN* matrix, Adafruit_NeoPixel* neo, RGB* color, bool writeFalse){
	int px = 0;
	#ifdef UpsideDown								// tausche oben und unten
			for(int row=9; row>=0; row--){			// Zeile (vertikal)
			#ifdef LeftRight						// tausche linke und rechte Seite
				for(int col=10; col>=0; col--){
					int shift;
					#ifdef LeftRightCable
						if(row%2 != 0){shift = 10-col;}else{shift = col;}
					#else
						shift = col;
					#endif
	...
	#else
		for(int row=0; row<10; row++){			// Zeile (vertikal)
			#ifdef LeftRight					// tausch linke und rechte Seite
				for(int col=10; col>=0; col--){
					int shift;
					#ifdef LeftRightCable
						if(row%2 != 0){shift = 10-col;}else{shift = col;}
